...
When running TechHub behind a reverse proxy or load balancer (for example AWS ALB, nginx, or Cloudflare), the proxy typically sets headers such as `X-Forwarded-For` or `X-Client-Ip` to surface the original client's IP address. These headers can be spoofed by untrusted clients unless the application only accepts them from known, trusted proxy IPs.
